...
const RACK_SIZES: RackUnitSize[] = [16, 21, 28, 36, 40, 44];
const RACK_DEPTHS: RackDepth[] = [60, 80, 100, 120];

export const AddRackModal: React.FC<AddRackModalProps> = ({ isOpen, onClose, onAddRack }) => {
  const { isEn, isRtl } = useLanguage();
  const [name, setName] = useState(isEn ? 'Rack-01' : 'رک-01');
  const [units, setUnits] = useState<RackUnitSize>(40);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition"
            />
          </div>

          {/* Unit Size Selection: 16, 21, 28, 36, 40, 44 */}
          <div className="space-y-2">
              ...
              {RACK_SIZES.map((size) => {
                ...
                return (
                  <button
                    ...
                    className={`py-3 px-2 rounded-xl text-center border font-bold text-sm transition-all flex flex-col items-center gap-1 ${
                      isSelected
                  ...
                  >
                    <span className="font-mono text-base">{size}U</span>
                    <span className="text-[10px] opacity-75 font-normal">{sizeLabel}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="flex items-center gap-3">
              {[
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-3 rounded-xl border text-xs flex items-center justify-center gap-2 transition ${
                    color === c.id
          ...
          <div className="pt-4 border-t border-slate-800 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white text-xs font-bold shadow-lg shadow-cyan-600/30 transition active:scale-95 flex items-center gap-2"
            >
