...
export const PortInspectorModal: React.FC<PortInspectorModalProps> = ({
  ...
  const handleApplyBatch = async () => {
    if (!device || selectedPortIds.length <= 1) return;
    try {
      ...
      const res = await batchUpdateSwitchPorts(device.id, selectedPortIds, updates);
      ...
      setBatchSuccessMessage(
        isEn
          ...
          : `تنظیمات با موفقیت روی ${res.updatedCount} پورت انتخابی اعمال شد!`
      );
      if (onPortUpdated) onPortUpdated();
      ...
      setError(err.message || 'Error applying batch changes');
    } finally {
